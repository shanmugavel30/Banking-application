...
public class AccountHolderView implements ControllerToView{
	private ViewToController acccontroller;
	private Scanner scan=new Scanner(System.in);
	
	public AccountHolderView() {
		acccontroller=new AccountHolderController(this);
	}
	public void choices(List<AccountHolder> account) {
		...
		if(opt==1) {
			...
			acccontroller.creditAmount(account,creditAmount);
		}
		else if(opt==3) {
			...
			acccontroller.debitAmount(account,debitAmount,pin);
		}
		else if(opt==4) {
			...
			//System.out.println();
			acccontroller.getTransations(account);
		}
		else {
